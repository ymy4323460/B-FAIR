...
if args.dataset == 'huawei':
    ...
    if args.dataset[:9] == 'logitdata' or args.dataset[:7] == 'invdata' or args.dataset[:3] == 'non':
        ...
        if args.dataset == 'movielens':
            train_dataset_dir = os.path.join(workstation_path, 'dataset', args.dataset, 'train', args.data_path)#_movie_tag
            test_dataset_dir = os.path.join(workstation_path, 'dataset', args.dataset, 'dev', args.data_path)
...
#                                             syn=syn, mode='test', nagetive_path=nagetive_path)
    if args.dataset == 'movielens':
        test_dataloader_auc_acc = dataload.dataload_ori(test_dataset_dir, 50, data_feature_path=data_feature_path,
                                            syn=syn, mode='test', shuffle=False, fairness=fair) #movie lens 3000 #syn 1000
    elif args.dataset == 'insurance':
        test_dataloader_auc_acc = dataload.dataload_ori(test_dataset_dir, 5, data_feature_path=data_feature_path,
                                            syn=syn, mode='test', shuffle=False, fairness=fair) #movie lens 3000 #syn 1000
    else:
        test_dataloader_auc_acc = dataload.dataload_ori(test_dataset_dir, 5, data_feature_path=data_feature_path,
                                            syn=syn, mode='test', shuffle=False, fairness=fair) #movie lens 3000 #syn 1000
...
#             total_test_fairness_mean = dict(zip(['auc', 'acc', 'ndcg', 'recall', 'precision'], [[] for k in range(5)]))
            ...
            for test_x, test_a, test_y, test_r, test_gu, test_ga in test_dataloader_auc_acc:
#                 print(test_y.numpy().sum())
                if args.dataset == 'movielens':
                    if test_y.numpy().sum() == 50 or test_y.numpy().sum() == 0:
                        test_dataloader_auc_acc_len -= 1
                        continue
                elif args.dataset == 'insurance':
                    if test_y.numpy().sum() == 5 or test_y.numpy().sum() == 0:
                        test_dataloader_auc_acc_len -= 1
                        continue
                if test_y.numpy().sum() == 5 or test_y.numpy().sum() == 0:
                    test_dataloader_auc_acc_len -= 1
                ...
                if args.downstream == 'MLP':
                    ...
                    if args.dataset == 'coat':
                        ...
                        top20 = np.repeat(np.sort(test_y_pred.reshape(-1, 50))[:, 10].reshape(-1, 1), 50, axis=1).reshape(-1)
                        test_ndcg = skm.ndcg_score(test_y.reshape(-1, 50), test_y_pred.reshape(-1, 50), k=10)
                    elif args.dataset == 'insurance':
                        top20 = np.repeat(np.sort(test_y_pred.reshape(-1, 5))[:, 3].reshape(-1, 1), 5, axis=1).reshape(-1)
                        test_ndcg = skm.ndcg_score(test_y.reshape(-1, 5), test_y_pred.reshape(-1, 5), k=3)
                    else:
                        top20 = np.repeat(np.sort(test_y_pred.reshape(-1, 5))[:, 3].reshape(-1, 1), 5, axis=1).reshape(-1)
                    ...
                    test_rec = skm.recall_score(test_y.cpu().numpy(), np.where(test_y_pred > top20, np.ones_like(test_y_pred),
                                                                          np.zeros_like(test_y_pred)))
                if args.dataset in ['movielens', 'insurance'] and args.test_sensitive_group == 'user':
                    if args.user_group_size > 2:
                        group_id = int(test_gu.cpu().numpy()[0])
                        group_len[int(group_id)] += 1
                    else:
                        group_id = 0 if test_gu[0]==-1 else 1
                        group_len[group_id] += 1

                elif args.dataset in ['movielens', 'insurance'] and args.test_sensitive_group == 'item':
                    group_id = int(test_ga.cpu().numpy()[0])
                    group_len[int(group_id)] += 1
                if args.dataset in ['movielens', 'insurance']:
#                     print(len(fair_test_dict['auc']))
            ...
            group_indicate = np.array(group_len) > 0
            group_len = list(np.array(group_len)[group_indicate])
            fair_test_dict['auc'] = list(np.array(fair_test_dict['auc'])[group_indicate])
            fair_test_dict['acc'] = list(np.array(fair_test_dict['acc'])[group_indicate])
            fair_test_dict['ndcg'] = list(np.array(fair_test_dict['ndcg'])[group_indicate])
            fair_test_dict['recall'] = list(np.array(fair_test_dict['recall'])[group_indicate])
            fair_test_dict['precision'] = list(np.array(fair_test_dict['precision'])[group_indicate])
            group_size = len(fair_test_dict['auc'])
#             print(fair_test_dict)
