...
class DataLoad(data.Dataset):
    def __init__(self, root, propensity=None, imputation=None, data_feature_path=None, a_propensity=False, Flag=False, mode='train', nagetive_path=None, group=False):
        # self.user_item = fileread(os.path.join(root,'data'))
        self.user_item = fileread(root)
        self.transforms = transforms.Compose([transforms.ToTensor()])
...
#                 print('xxxxxxxxxxxxxxxxxxxx')
            ...
            if self.mode == 'train':
                if self.group:
                    return data[0], data[1], data[2], data[3], data[4], data[5], propensity_data
                return data[0], data[1], data[2], data[3], propensity_data
            else:
                if self.group:
                    return data[0], data[1], data[2], data[3], data[4], data[5], propensity_data, self.negative_data
...
def dataload(dataset_dir, batch_size, shuffle=False, num_workers=8, pin_memory=True, propensity=None, imputation=None,
             data_feature_path=None, a_propensity=False, Flag=False, mode='train', nagetive_path=None, group=False):
    dataset = DataLoad(dataset_dir, propensity, imputation, data_feature_path, a_propensity, Flag, mode=mode, nagetive_path=nagetive_path, group=group)
    dataset = Data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=8, pin_memory=True)
    return dataset
