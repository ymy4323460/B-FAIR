...
#         ga = np.load('./nonlinear{}data_{}_{}/item_group.npy'.format(policy, sample_num, context_dim), allow_pickle=True)
    ...
    np.save('./nonlinearinvdata_{}_{}/user_group'.format(sample_num, context_dim), gx)
    np.save('./nonlinearinvdata_{}_{}/item_group'.format(sample_num, context_dim), ga)
...
for sam in [25, 50]:
    for cdim in [16, 32]:
        logit_impression_list_new(mode='dev', step=sam,  impression_len=10, policy='nonlinearinv', sample_num=sam, context_dim=cdim)
        ...
        random_impression_list(mode='dev', step=sam,  impression_len=10, policy='nonlinearinv', sample_num=sam, context_dim=cdim)
        fair_impression_list(mode='dev', step=sam,  impression_len=10, policy='nonlinearinv', sample_num=sam, context_dim=cdim)
