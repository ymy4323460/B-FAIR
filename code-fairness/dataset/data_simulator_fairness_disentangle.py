...
#     print(gx[x_g_idx])
    ...
    sample = np.random.choice(np.arange(int(sample_size * 0.75)), size=int(sample_size * 0.25), replace=True)
    x = np.concatenate((x[:int(sample_size * 0.75)], x[:int(sample_size * 0.75)][sample]), axis=0)
    x = np.concatenate((x, gx[x_g_idx]), axis=1)
    a = np.concatenate((np.random.uniform(-1, 1, (item_size, context_dim)), ga[a_g_idx]), axis=1)

    x = 4*(sigmoid_fun(x) - 0.5)
    print(x.max(), x.min())

    w = np.random.uniform(-1, 1, x.shape[1] * x.shape[1]).reshape(x.shape[1],-1)
    w = np.triu(w,1)
    w = w - np.diag(w)

    w = w + np.identity(x.shape[1])

    x = 4.5*(sigmoid_fun(x) - 0.5)


    indicate = np.where(x > 0, 1, 0)
    indicate_neg = np.where(x < 0, 1, 0)
    pos = np.multiply(x, indicate) - 0.5 * indicate

    neg = np.multiply(x, indicate_neg) + 0.5 * indicate

#     x = np.array(list(map(entangle, x, [w for j in range(x.shape[0])])))
# #     x = np.clip(x, -1, 1)
#     x = x / np.absolute(x).max()
#     print(x)

# #     print((pos+neg).shape)
    x = 4.5*(sigmoid_fun(pos+neg)-0.5)#+2*(sigmoid_fun(neg)-0.5)
#     print('haha', x.shape)
    ...
    return x, x_g_idx, a, a_g_idx, gx, ga
...
def get_impression_feature(x, parameters=None):
    item_feature_dict = np.load('./{}data_{}_{}/item_features.npy'.format(parameters[0], parameters[1], parameters[2]),
...
def logit_impression_list_new(context_dim=32, impression_len=5, sample_size=128, item_size=32, step=10, mode='dev',
    ...
    x, gx, a, ga, g_u_feat, g_a_feat = user_item_feature(context_dim=context_dim, sample_size=sample_num * 200, item_size=item_size)
#     if mode == 'train':
...
#         gx = np.load('./nonlinear{}data_{}_{}/user_group.npy'.format(policy, sample_num, context_dim), allow_pickle=True)
#         ga = np.load('./nonlinear{}data_{}_{}/item_group.npy'.format(policy, sample_num, context_dim), allow_pickle=True)
    user_feature_dict = dict(zip([j for j in range(sample_size)], list(x)))
    item_feature_dict = dict(zip([j for j in range(item_size)], list(a)))
    user_feature_ori_dict = dict(zip([j for j in range(9)], list(g_u_feat)))
    item_feature_ori_dict = dict(zip([j for j in range(3)], list(g_a_feat)))
    # print(x)
    if not os.path.exists('./{}data_{}_{}/'.format(policy, sample_num, context_dim)):  # 判断所在目录下是否有该文件名的文件夹
        os.makedirs('./{}data_{}_{}/dev/'.format(policy, sample_num, context_dim))
        os.makedirs('./{}data_{}_{}/train/'.format(policy, sample_num, context_dim))
    np.save('./{}data_{}_{}/user'.format(policy, sample_num, context_dim), x)
    np.save('./{}data_{}_{}/item'.format(policy, sample_num, context_dim), a)
    np.save('./{}data_{}_{}/user_group'.format(policy, sample_num, context_dim), gx)
    np.save('./{}data_{}_{}/item_group'.format(policy, sample_num, context_dim), ga)
    np.save('./{}data_{}_{}/user_features'.format(policy, sample_num, context_dim), user_feature_dict)
    np.save('./{}data_{}_{}/item_features'.format(policy, sample_num, context_dim), item_feature_dict)
    np.save('./{}data_{}_{}/user_features_group'.format(policy, sample_num, context_dim), user_feature_ori_dict)
    np.save('./{}data_{}_{}/item_features_group'.format(policy, sample_num, context_dim), item_feature_ori_dict)
    ...
    pd.DataFrame(all_[: partition]).to_csv(
        './{}data_{}_{}/dev/data_nonuniform.csv'.format(policy, sample_num, context_dim), header=False, index=False)
    pd.DataFrame(all_[partition:]).to_csv(
        './{}data_{}_{}/train/data_nonuniform.csv'.format(policy, sample_num, context_dim), header=False, index=False)
    return all_, user_feature_dict, item_feature_dict
...
def fair_impression_list(context_dim=32, impression_len=5, sample_size=128, item_size=32, step=10, mode='dev',
    ...
    partition = int(0.75 * all_.shape[0])
    pd.DataFrame(all_[: partition]).to_csv(
        './{}data_{}_{}/dev/data_fair.csv'.format(policy, sample_num, context_dim, mode), header=False,
        index=False)
    pd.DataFrame(all_[partition:]).to_csv(
        './{}data_{}_{}/train/data_fair.csv'.format(policy, sample_num, context_dim, mode), header=False, index=False)
...
for sam in [25]:
    for cdim in [32]:
        logit_impression_list_new(mode='dev', step=sam,  impression_len=5, policy='nonlinearinvdisn', sample_num=sam, context_dim=cdim)
        ...
        random_impression_list(mode='dev', step=sam,  impression_len=5, policy='nonlinearinvdisn', sample_num=sam, context_dim=cdim)
        fair_impression_list(mode='dev', step=sam,  impression_len=5, policy='nonlinearinvdisn', sample_num=sam, context_dim=cdim)
