...
class EmbeddingLayer(nn.Module):
    ...
    def predict(self, x, a=None):
        if self.x_dim > 1:
            ...
            x_emb = self.embedding_lookup(x).reshape([x.size()[0], self.emb_dim])
            a_emb = self.embedding_lookup(a).reshape([x.size()[0], self.emb_dim])
            x_embedding = torch.concat()
        return x_embedding

class DeEmbeddingLayer(nn.Module):
    def __init__(self, name, emb_dim, x_dim, x_size=None):
        super().__init__()
        self.name = 'DeEmbedding' + str(name)
        self.x_dim = x_dim
        self.emb_dim = emb_dim
    ...
    def predict(self, x):
        x_embedding = self.embedding(x).reshape([x.size()[0], self.x_dim])
        return x_embedding

class Disentangle(nn.Module):
    def __init__(self, x_dim, a_dim, emb_dim1, emb_dim2, x_a_size=None, discriminitor=None, confounder_dims=None, embedding_classweight=None):
        super().__init__()
        ...
        self.embedding_rec_sensitive_x = DeEmbeddingLayer('gx_sensitive', int(emb_dim1/2), x_dim)
        self.embedding_rec_sensitive_a = DeEmbeddingLayer('ga_sensitive', int(emb_dim1/2), x_dim)
        self.embedding_rec_non_sensitive = DeEmbeddingLayer('non_sensitive', emb_dim1, x_dim)
    ...
    def disentangle_rep(self, x, a=None):
        if self.x_dim > 1:
            ...
            sensitive_ga = self.embedding_sensitive_a.predict(x, a)
            sensitive_non = self.embedding_non_sensitive.predict(x, a)
        return sensitive_gx, sensitive_ga, sensitive_non

    def reconstruct(self, sensitive_gx, sensitive_ga, sensitive_non):
        x1 = self.embedding_rec_sensitive_x.predict(sensitive_gx)
        x2 = self.embedding_rec_sensitive_a.predict(sensitive_ga)
        x3 = self.embedding_rec_non_sensitive.predict(sensitive_non)
        return x1, x2, x3

    def discepency(self, x, gx, ga, a=None):
        sensitive_gx, sensitive_ga, sensitive_non = self.disentangle_rep(x)
        ...
        x1, x2, x3 = self.reconstruct(sensitive_gx, sensitive_ga, sensitive_non)
        ...
        kl = kl_x.mean() + kl_a.mean() + kl_non
        rec = self.mse(x1, x) + self.mse(x2, x) + self.mse(x3, x)
        return sensitive_gx, sensitive_ga, sensitive_non, kl.mean() + rec
...
class FairnessRep(nn.Module):
    ...
    def get_fair_emb(self, x):
        sensitive_gx, sensitive_ga, sensitive_non = self.disentangle.disentangle_rep(x)
    ...
    def discepency(self, p_x, a, gx, ga, debias_mode='impression', z=None, r_true=None):
        ...
        if debias_mode == 'Adversarial':
            ...
            x_embedding = self.emb_with_disentangle(torch.concat((sensitive_non, sensitive_non), dim=1))
#             x_embedding = self.emb_with_disentangle(torch.concat((sensitive_gx, sensitive_ga, sensitive_non), dim=1))
            if self.a_dim > 1:
                a_embedding = self.item_embedding(a).reshape([a.size()[0], self.a_emb_dim])
            else:
