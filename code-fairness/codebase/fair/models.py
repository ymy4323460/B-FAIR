...
class EmbeddingLayer(nn.Module):
    def __init__(self, name, x_dim, emb_dim, x_size=None):
        super().__init__()
        self.name = 'Embedding' + str(name)
        self.x_dim = x_dim
#         print(x_size)
        self.emb_dim = emb_dim
        if x_size is not None:
            self.embedding_lookup_x =  nn.Sequential(
                nn.Embedding(x_size[0], emb_dim),
                nn.ReLU(),
                nn.Linear(emb_dim, int(self.emb_dim/2))
            )
            self.embedding_lookup_a =  nn.Sequential(
                nn.Embedding(x_size[1], emb_dim),
                nn.ReLU(),
                nn.Linear(emb_dim, int(self.emb_dim/2))
            )
        else:
    ...
    def predict(self, x, a=None):
        if self.x_dim > 1:
            x_embedding = self.embedding(x).reshape([x.size()[0], self.emb_dim])
        else:
            x_emb = self.embedding_lookup_x(x).reshape([x.size()[0], int(self.emb_dim/2)])
            a_emb = self.embedding_lookup_a(a).reshape([x.size()[0], int(self.emb_dim/2)])
            x_embedding = torch.concat([x_emb, a_emb], axis=1)
        return x_embedding

class Disentangle(nn.Module):
    def __init__(self, x_dim, a_dim, emb_dim1, emb_dim2, x_a_size=None, discriminitor=None, confounder_dims=None, embedding_classweight=None, no_disentangle=False):
        super().__init__()
        self.name = 'Disentangle'
        self.no_disentangle = no_disentangle
        self.x_dim = x_dim
        self.a_dim = a_dim
...
#         self.self.no_disentangle = no_disentangle
        self.confounder_dims = confounder_dims
        self.embedding_sensitive_x = EmbeddingLayer('gx_sensitive', x_dim, int(emb_dim1/2), x_a_size)
        self.embedding_sensitive_a = EmbeddingLayer('ga_sensitive', x_dim, int(emb_dim1/2), x_a_size)
        self.embedding_non_sensitive = EmbeddingLayer('non_sensitive', x_dim, emb_dim1, x_a_size)

    def condition_prior(self, label, dim, scale=[0,1]):
        mean = torch.ones(label.size()[0], dim)
        var = torch.ones(label.size()[0], dim)
        # print(label)
        mul = (label-scale[0])/(scale[1]-0)
        mean = mul
        var = torch.ones(dim).to(device)
...
#             var[i] = torch.ones(dim)*1
    ...
    def disentangle_rep(self, x, a=None):
        if self.x_dim > 1:
            sensitive_gx = self.embedding_sensitive_x.predict(x)
            sensitive_ga = self.embedding_sensitive_a.predict(x)
            sensitive_non = self.embedding_non_sensitive.predict(x)
        else:
            sensitive_gx = self.embedding_sensitive_x.predict(x, a)
            sensitive_ga = self.embedding_sensitive_x.predict(x, a)
            sensitive_non = self.embedding_non_sensitive.predict(x, a)
        return sensitive_gx, sensitive_ga, sensitive_non

    def discepency(self, x, gx, ga, a=None):
        sensitive_gx, sensitive_ga, sensitive_non = self.disentangle_rep(x, a)

        if self.no_disentangle:
#             prior_gx, v = torch.zeros((gx, sensitive_gx.size()[1]), torch.ones(gx, sensitive_gx.size()[1]))
#             prior_ga, v = torch.zeros((gx, sensitive_ga.size()[1]), torch.ones(ga, sensitive_ga.size()[1]))
#             prior_gx_non, v_non = torch.zeros((gx, sensitive_gx.size()[1]), torch.ones(gx, sensitive_gx.size()[1]))
#             prior_ga_non, v_non = torch.zeros((gx, sensitive_gx.size()[1]), torch.ones(gx, sensitive_gx.size()[1]))
#             kl_non = (ut.kl_normal(sensitive_non, v_non, prior_ga_non, v_non).mean() + ut.kl_normal(sensitive_non, v_non, prior_gx_non, v_non).mean())
            kl_non = 0
            kl_gamma1 = 0
            kl_gamma1 = 0
        else:
            prior_gx, v = self.condition_prior(gx, sensitive_gx.size()[1])
            prior_ga, v = self.condition_prior(ga, sensitive_ga.size()[1])
            prior_gx_non, v_non = self.condition_prior(gx, sensitive_non.size()[1])
            prior_ga_non, v_non = self.condition_prior(ga, sensitive_non.size()[1])
            kl_non = -(ut.kl_normal(sensitive_non, v_non, prior_ga_non, v_non).mean() + ut.kl_normal(sensitive_non, v_non, prior_gx_non, v_non).mean())
            kl_gamma1 = 0.1
            kl_gamma2 = 0.01
        kl_x = ut.kl_normal(sensitive_gx, v, prior_gx, v)
        kl_a = ut.kl_normal(sensitive_ga, v, prior_ga, v)

#
        kl = kl_x.mean() + kl_a.mean() + kl_gamma1*kl_non
        return sensitive_gx, sensitive_ga, sensitive_non, kl_gamma2*kl.mean()

class FairnessRep(nn.Module):
    def __init__(self, x_dim, a_dim, emb_dim1, emb_dim2, layer_dim, item_space, x_a_size=None, discriminitor=None, confounder_dims=None, embedding_classweight=None):
        ...
        self.confounder_dims = confounder_dims
        if a_dim > 1:
            self.disentangle = Disentangle(x_dim, a_dim, emb_dim1, emb_dim2)
        else:
            self.disentangle = Disentangle(x_dim, a_dim, emb_dim1, emb_dim2, x_a_size=x_a_size)
        ...
        self.emb_with_disentangle = nn.Sequential(
            nn.Linear(self.x_emb_dim*2, self.x_emb_dim),
            nn.ELU()
        )
    ...
    def get_fair_emb(self, x, a=None):
        sensitive_gx, sensitive_ga, sensitive_non = self.disentangle.disentangle_rep(x, a)
        x_embedding = self.emb_with_disentangle(torch.concat((sensitive_gx, sensitive_ga, sensitive_non), dim=1))
        return x_embedding

    def discepency(self, p_x, a, gx, ga, debias_mode='impression', z=None, r_true=None):
        '''
        ...
        elif debias_mode == 'Fairness':
            if self.a_dim > 1:
                sensitive_gx, sensitive_ga, sensitive_non, kl = self.disentangle.discepency(p_x, gx, ga)
            else:
                sensitive_gx, sensitive_ga, sensitive_non, kl = self.disentangle.discepency(p_x, gx, ga, a)
#             print('testtest', sensitive_gx.size())
...
#             x_embedding = self.emb_with_disentangle(torch.concat((sensitive_gx, sensitive_ga, sensitive_non), dim=1))
            if self.a_dim > 1:
                ...
                a_embedding = self.item_embedding_lookup(a).reshape([a.size()[0], self.a_emb_dim])
            discrepency = -self.discriminitor.loss(torch.concat((sensitive_gx, sensitive_ga), dim=1), a_embedding, r_true)
            x_embedding = self.emb_with_disentangle(torch.concat((sensitive_gx, sensitive_ga, sensitive_non), dim=1))
            return x_embedding, discrepency, kl, sensitive_gx, sensitive_ga, sensitive_non

        return x_embedding, discrepency.mean(), kl
...
class IPMEmbedding(nn.Module):
    ...
    def discepency(self, p_x, a, debias_mode='impression', z=None, r_true=None):
        '''
        p_x: (batch_size, feature size)
        a: (batch_size, index)
        a_hat: (batch_size, item_space_size)
...
class DoubleModelCTR(nn.Module):
    ...
    def predict(self, x, a):
        ...
        elif self.mode in ['IPM_Embedding', 'Catboost', 'Adversarial', 'Fairness']:
            if self.is_debias:
...
class DirectMethod(nn.Module):
    ...
    def weighted_loss(self, x, a, y, w):
        y = torch.tensor(y, dtype=torch.float32).to(device)
        w = torch.ones_like(y)#torch.tensor(1.0/torch.clamp(w, 0.3, 1), dtype=torch.float32).to(device)
